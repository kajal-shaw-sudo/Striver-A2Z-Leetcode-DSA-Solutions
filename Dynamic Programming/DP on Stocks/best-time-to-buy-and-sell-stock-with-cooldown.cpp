...
private:
    int helper(vector<int>& prices, int i, int buy, int n) {
        ...
        if (buy == 0) {
            profit = max(0 + helper(prices, i+1, 0, n), -prices[i] + helper(prices, i+1, 1, n));
        }

        if (buy == 1) {
            profit = max(0 + helper(prices, i+1, 1, n), prices[i] + helper(prices, i+2, 0, n));
        }
...
private:
    int helper(vector<int>& prices, int i, int buy, int n, vector<vector<int>>& dp) {
        ...
        if (buy == 0) {
            profit = max(0 + helper(prices, i+1, 0, n, dp), -prices[i] + helper(prices, i+1, 1, n, dp));
        }

        if (buy == 1) {
            profit = max(0 + helper(prices, i+1, 1, n, dp), prices[i] + helper(prices, i+2, 0, n, dp));
        }
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        vector<int> curr(2, 0), front(2, 0), front2(2, 0);
        ...
        for (int i=n-1; i>=0; i--) {
            for (int buy=0; buy<=1; buy++) {
                ...
                if (buy == 1) {
                    profit = max(0 + front[1], prices[i] + front2[0]);
                }
